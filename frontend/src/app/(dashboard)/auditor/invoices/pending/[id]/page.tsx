...
import {
    ...
    Link2, ShieldCheck, ShieldAlert, Ban, Activity, AlertCircle,
...
export default function PendingInvoiceDetail() {
    ...
    const fields = [
        ...
        { icon: User,      label: "Submitted By", value: inv.uploadedBy?.name ?? "—"                   },
        { icon: Coins,     label: "Amount",       value: formatCurrency(inv.amount)                     },
        { icon: Calendar,  label: "Uploaded",     value: formatDate(inv.createdAt)                      },
    ];
    ...
    return (
        <div className="space-y-8 max-w-2xl">
            ...
            {inv.blockchainTxHash && (
                <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.14 }} className="mg-card rounded-2xl p-6">
                    <div className="flex items-center gap-2 mb-3">
                        <ShieldCheck className="w-4 h-4 text-status-success" />
                        <p className="mg-label">On-Chain Registration TX</p>
                    </div>
            ...
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="mg-card rounded-2xl p-6">
                ...
                {logs.length === 0 ? (
                    <p className="text-sm text-mg-dim italic text-center py-6">No audit events yet</p>
                ) : (
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.25 }}
                className="flex items-center gap-3 p-4 rounded-xl bg-mg-elevated border border-mg-lavender/10">
                <ShieldAlert className="w-4 h-4 text-mg-lavender shrink-0" />
                <p className="text-xs text-mg-muted">
                    Auditor view is <span className="font-semibold text-mg-silver">read-only</span>. Invoice actions are performed by lenders.
                </p>
                <Ban className="w-4 h-4 text-mg-dim shrink-0 ml-auto" />
            </motion.div>
        </div>
    );
}
